...
public class BookingIDsGet implements Task {

    public BookingIDsGet() {
    }

    public static Performable withId() {
        return Instrumented.instanceOf(BookingIDsGet.class).withProperties();
    }
    ...
    public <T extends Actor> void performAs(T actor) {
            actor.attemptsTo(
                    Get.resource(
                            EnvironmentProperties.getProperty(BOOKING_IDS)
                    )
            );
    }
}